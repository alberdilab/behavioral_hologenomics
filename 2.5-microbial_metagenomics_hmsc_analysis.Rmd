---
title: "2.5-microbial_metagenomics_hmsc_analysis"
...
```{r hmsc_variancepart, warning=FALSE, comments="", message=FALSE, fig.height=6, fig.width=10, fig.fullwidth=TRUE, eval=FALSE}
# Select modelchain of interest
varpart=computeVariancePartitioning(m)
plotVariancePartitioning(hM=m,VP=varpart)
```

The summary statistics indicate that sequencing depth is the variable that explains most of the variance. This is because the three sequencing batches aimed for different sequencing depths, the first batch being much more deeply sequenced than the rest, with the objective of generating the MAG catalogue. The cage and the animal random effects explain a considerable fraction of the variation, indicating that the microbiomes of animals throughout time and within cage mates are more alike, which highlights the importance of controlling for these features when modelling. The two variables that explain the lowest portion of the variance are the experimental group (variable or invariable environment) and dominance index, highlighting that the effect sizes of dominance are rather low.
